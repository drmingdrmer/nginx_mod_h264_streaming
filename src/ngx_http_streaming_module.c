...
static ngx_int_t open_file(char const* filename, ngx_http_request_t *r, ngx_http_core_loc_conf_t *clcf, ngx_str_t *path, ngx_open_file_info_t *of)
{
  ngx_uint_t level;
  ngx_int_t rc;
  ngx_log_t *log;
...
static ngx_int_t ngx_streaming_handler(ngx_http_request_t *r)
{
...

  {
    ...
    rc = open_file(filename, r, clcf, &path, &of);
    ...
    buckets_t* buckets = buckets_init();
    int verbose = 0;
    int http_status =
      mp4_process(filename, get_filesize(filename), verbose, buckets, options);
    ...
    r->headers_out.content_type.len = strlen(buckets->content_type_);
    r->headers_out.content_type.data = (u_char*)buckets->content_type_;
          ...
          *chain = ngx_pcalloc(r->pool, sizeof(ngx_chain_t));
          ...
          case BUCKET_TYPE_FILE:
            ...
            else
            {
              ...
              rc = open_file(bucket->filename_, r, clcf, &path, &of);
          ...
          b->last_buf = bucket->next_ == buckets->bucket_ ? 1 : 0;
          b->last_in_chain = bucket->next_ == buckets->bucket_ ? 1 : 0;
