...
extern void trak_shift_offsets_inplace(trak_t* trak, int64_t offset)
{
  unsigned char* buffer = trak->mdia_->minf_->stbl_->stco_->stco_inplace_;
  int co64 = read_32(buffer) == FOURCC('c', 'o', '6', '4');
  unsigned char* stco = buffer + 4;
  ...
  if(co64)
  {
    int64_t* table = (int64_t*)(stco + 8);
    for(i = 0; i != entries; ++i)
    {
      int64_t new_offset = (read_64((unsigned char*)&table[i]) + offset);
      write_64((unsigned char*)&table[i], new_offset);
    }
  }
  else
  {
    ...
    for(i = 0; i != entries; ++i)
    {
      int64_t new_offset = (read_32((unsigned char*)&table[i]) + offset);
      // TODO: ASSERT(new_offset <= UINT32_MAX);
      write_32((unsigned char*)&table[i], (uint32_t)new_offset);
    }
  }
}
...
static unsigned char* stco_write(void const* atom, unsigned char* buffer)
{
  ...
  ((stco_t*)stco)->stco_inplace_ = buffer - 4;

  co64 = stco->entries_ &&
         stco->chunk_offsets_[stco->entries_ - 1] > UINT32_MAX;

  if(co64)
  {
    write_32(buffer - 4, FOURCC('c', 'o', '6', '4'));
  }
  ...
  for(i = 0; i != stco->entries_; ++i)
  {
    if(co64)
    {
      buffer = write_64(buffer, stco->chunk_offsets_[i]);
    }
    else
    {
...
extern unsigned char* uuid0_write(void const* atom, unsigned char* buffer)
{
...
extern unsigned char* uuid1_write(void const* atom, unsigned char* buffer)
{
