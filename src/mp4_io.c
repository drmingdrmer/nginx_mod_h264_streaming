...
char const* mime_type_flv = "video/x-flv";
char const* mime_type_ismc = "text/xml";
char const* mime_type_mp4 = "video/mp4";
char const* mime_type_ts = "video/MP2T";
                                   ...
                                   uint64_t offset, uint64_t len)
{
  ...
  if(mem_range->fd_ == -1)
  {
    printf("mem_range: Error opening file %s (%s)\n", filename, strerror(errno));
...
//    printf("mem_range(%x): offset=%"PRIu64"\n", mem_range, offset);
    ...
    if(!mem_range->mmap_addr_)
    {
      printf("Unable to make file mapping. offset=%"PRIu64" size=%"PRIu64"\n",
             mem_range->mmap_offset_, mem_range->mmap_size_);
      return 0;
    }
#else
    ...
    {
      printf("Unable to make file mapping. offset=%"PRIu64" size=%"PRIu64" (%s)\n",
             mem_range->mmap_offset_, mem_range->mmap_size_, strerror(errno));
      mem_range->mmap_addr_ = NULL;
...
{
  unsigned char atom_header[ATOM_PREAMBLE_SIZE];

  atom->start_ = ftello(infile);
  if(fread(atom_header, ATOM_PREAMBLE_SIZE, 1, infile) != 1)
  {
    MP4_ERROR("%s", "Error reading atom header\n");
    return 0;
  }
  atom->size_ = read_32(&atom_header[0]);
...
{
  int write_box64 = atom->size_ > UINT32_MAX ? 1 : 0;
...
extern mp4_context_t* mp4_context_init(const char* filename, int verbose)
{
  mp4_context_t* mp4_context = (mp4_context_t*)malloc(sizeof(mp4_context_t));

  mp4_context->filename_ = filename ? strdup(filename) : NULL;
...
extern void mp4_context_exit(struct mp4_context_t* mp4_context)
{
  if(mp4_context->filename_)
  {
  ...
  if(mp4_context->ftyp_data)
  {
    free(mp4_context->ftyp_data);
  }
