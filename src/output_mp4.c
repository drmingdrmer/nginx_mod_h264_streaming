...
static void trak_shift_offsets(trak_t* trak, int64_t offset)
{
...
static void moov_shift_offsets_inplace(moov_t* moov, int64_t offset)
{
  unsigned int i;
...
static void moov_shift_offsets(moov_t* moov, int64_t offset)
{
  unsigned int i;
  for(i = 0; i != moov->tracks_; ++i)
  {
    ...
    if(stsc != NULL)
    {
...

      {
...

        {
          ...
          stco->chunk_offsets_[0] = trak->samples_[start].pos_;
        }
      }
    }
  }
...
//  unsigned char* moov_data = mp4_context->moov_data;
  ...
  uint64_t new_mdat_start = 0;
  {
    ...
    {
      memcpy(buffer, mp4_context->ftyp_data,
             (size_t)mp4_context->ftyp_atom.size_);
    }
    ...
    {
      bucket_t* bucket = bucket_init_memory(buckets, buffer, size_of_header);
...
//      MP4_INFO("stsc.samples=%d\n", stsc_get_samples(stbl->stsc_));
  ...
  MP4_INFO("shifting offsets by %"PRId64"\n", moov_size);
  moov_shift_offsets_inplace(moov, moov_size);
  ...
  bucket_insert_tail(buckets, bucket_init_memory(buckets, moov_data, moov_size));
  free(moov_data);

  {
...

    {
      ...
      bucket_insert_tail(buckets,
        bucket_init_memory(buckets, buffer, mdat_header_size));

      if(mdat_atom.size_ - mdat_header_size)
      {
        bucket_insert_tail(buckets,
          bucket_init_file(buckets, mp4_context->filename_,
