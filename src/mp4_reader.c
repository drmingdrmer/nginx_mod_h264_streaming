...
{
  atom->start_ = buffer;
  atom->size_ = read_32(buffer);
  atom->type_ = read_32(buffer + 4);
  buffer += 8;

  if(atom->size_ == 1)
  {
    atom->size_ = read_64(buffer);
    buffer += 8;
  }
                       ...
                       unsigned char* buffer, uint64_t size)
{
  ...
  if(result)
  {
    result = moov_build_index(mp4_context, atom);
  }
...
static unsigned char* read_box(struct mp4_context_t* mp4_context,
                               struct mp4_atom_t* atom)
{
  unsigned char* box_data = (unsigned char*)malloc((size_t)atom->size_);
  fseeko(mp4_context->infile, atom->start_, SEEK_SET);
  if(fread(box_data, (off_t)atom->size_, 1, mp4_context->infile) != 1)
  {
                               ...
                               mp4_open_flags flags, int verbose)
{
  ...
  while(ftello(mp4_context->infile) < filesize)
  {
    ...
    {
    case FOURCC('f', 't', 'y', 'p'):
      if(mp4_context->ftyp_data != NULL)
      {
        MP4_ERROR("multiple ftyp atoms (%s)\n", filename);
        mp4_context_exit(mp4_context);
        return 0;
      }

      mp4_context->ftyp_atom = leaf_atom;
      mp4_context->ftyp_data = read_box(mp4_context, &mp4_context->ftyp_atom);
      if(mp4_context->ftyp_data == NULL)
      {
        mp4_context_exit(mp4_context);
        return 0;
    ...
    case FOURCC('m', 'o', 'o', 'v'):
      ...
      {
        MP4_ERROR("parsing moov header (%s)\n", filename);
        mp4_context_exit(mp4_context);
        return 0;
      }
      break;
    case FOURCC('m', 'o', 'o', 'f'):
      {
        ...
        if(mp4_context->moov == NULL)
        {
          MP4_ERROR("moof atom must be preceded by moov atom (%s)\n", filename);
          mp4_context_exit(mp4_context);
          return 0;
        }

        moof_data = read_box(mp4_context, &leaf_atom);
        if(moof_data == NULL)
        {
          mp4_context_exit(mp4_context);
          return 0;
        ...
        if(moof == 0)
        {
          MP4_ERROR("parsing moof header (%s)\n", filename);
          mp4_context_exit(mp4_context);
          return 0;
        }
    ...
    case FOURCC('m', 'f', 'r', 'a'):
      ...
      mp4_context->mfra_data = read_box(mp4_context, &mp4_context->mfra_atom);
    ...
    {
      MP4_ERROR("Reached end of file prematurely (%s)\n", filename);
      mp4_context_exit(mp4_context);
      return 0;
    }
  ...
  if(mp4_context->moov == 0)
  {
    MP4_ERROR("moov atom not found (%s)\n", filename);
    mp4_context_exit(mp4_context);
    return 0;
  }
  ...
  {
    MP4_ERROR("mdat atom not found (%s)\n", filename);
    mp4_context_exit(mp4_context);
    return 0;
  }
